...
def main():
    ...
    opc_count = 0
    object_id_list = []
    alarm_triggered = False  # Flag to track if alarm has been triggered
    while True:
        ret, frame = cap.read()
        ...
        # Check if LPC count exceeds the threshold
        if lpc_count >= 30 and not alarm_triggered:
            cv2.putText(frame, "ALARM: LPC limit exceeded!", (5, 120), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 0, 255),
                        1)
            alarm_triggered = True  # Set the alarm flag to prevent multiple triggers
            alarm_sound.play()
